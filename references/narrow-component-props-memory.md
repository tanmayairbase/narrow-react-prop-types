...
- For controls that are interactive unless explicitly read-only, use a union that requires the handler in the interactive variant and permits its omission only with `disabled: true`.
